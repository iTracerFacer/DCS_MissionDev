--env.info("Loading Naval Script", true)
-- Expected Behavior
...
local Tarawa_Carrier_Group = GROUP:FindByName("LHA-1 Tarawa")
local Tarawa_Beacon_Unit = UNIT:FindByName("LHA-1 Tarawa")
local Tarawa_ICLS_Channel = 1
local Tarawa_ICLS_Name = "LHA"
local Tarawa_TACAN_Channel = 4
local Tarawa_TACAN_Name = "LHA"
local Tarawa_RecoveryWindowTime = 20


-- Create a NAVYGROUP object and activate the late activated group.
BlueLHAGroup=NAVYGROUP:New("LHA-1 Tarawa")  
BlueLHAGroup:SetVerbosity(1)
BlueLHAGroup:MarkWaypoints()

-- Function to turn ship into the wind.
function LHA_start_recovery()
  
  if BlueLHAGroup:IsSteamingIntoWind() == true then
    MESSAGE:New(Tarawa_Beacon_Unit:GetName() .. " is currently launching/recovering aircraft, currently active recovery window closes at time " .. timerecovery_end, msgTime, "LHANAVINFO",false):ToBlue()
    USERSOUND:New("ping.ogg"):ToCoalition(coalition.side.BLUE)
  else
    local timenow=timer.getAbsTime( )
    local timeend=timenow+Tarawa_RecoveryWindowTime*60 -- this sets the recovery window to 45 minutes, you can change the numbers as you wish
    local timerecovery_start = UTILS.SecondsToClock(timenow,true)
    timerecovery_end = UTILS.SecondsToClock(timeend,true)
    BlueLHAGroup:AddTurnIntoWind(timerecovery_start,timerecovery_end,25,true,-9)
    MESSAGE:New(Tarawa_Beacon_Unit:GetName().." is turning into the wind to begin " .. Tarawa_RecoveryWindowTime .. " mins of aircraft operations.\nLaunch/Recovery Window will be open from time " .. timerecovery_start .. " until " .. timerecovery_end, 35, "LHANAVINFO",false):ToBlue()
    USERSOUND:New("ping.ogg"):ToCoalition(coalition.side.BLUE)
  end
end

--- Function called each time the group passes a waypoint.
function BlueLHAGroup:OnAfterPassingWaypoint(From, Event, To, Waypoint)
  local waypoint=Waypoint --Ops.OpsGroup#OPSGROUP.Waypoint
  
  -- Debug info.
  local text=string.format(Tarawa_Beacon_Unit:GetName().." passed waypoint ID=%d (Index=%d) %d times", waypoint.uid, BlueLHAGroup:GetWaypointIndex(waypoint.uid), waypoint.npassed)
  MESSAGE:New(text, msgTime):ToBlue()
  USERSOUND:New("ping.ogg"):ToCoalition(coalition.side.BLUE)
  env.info(text)
--- Patrol zone 1  
  if waypoint == 0 then
    MESSAGE:New(Tarawa_Beacon_Unit:GetName().. "proceeding to waypoint 1", msgTime):ToBlue()
    USERSOUND:New("ping.ogg"):ToCoalition(coalition.side.BLUE)
    BlueLHAGroup:GotoWaypoint(1)
  end
  
  if waypoint == 2 then
    MESSAGE:New(Tarawa_Beacon_Unit:GetName().. "proceeding to waypoint 2", msgTime):ToBlue()
    USERSOUND:New("ping.ogg"):ToCoalition(coalition.side.BLUE)    
    BlueLHAGroup:GotoWaypoint(0)
  end
...
if Tarawa_Beacon_Unit then
  BlueTarawaGroup_Beacon = Tarawa_Beacon_Unit:GetBeacon()
  BlueTarawaGroup_Beacon:ActivateICLS(Tarawa_ICLS_Channel,Tarawa_ICLS_Name)
end
SCHEDULER:New(nil,function()
  if Tarawa_Beacon_Unit then
    BlueTarawaGroup_Beacon = Tarawa_Beacon_Unit:GetBeacon()
    BlueTarawaGroup_Beacon:ActivateTACAN(Tarawa_TACAN_Channel,"X",Tarawa_TACAN_Name,true)
  end
end,{},5,5*60)


-- Function to turn ship into the wind.
function CVN_start_recovery()
...
MENU_COALITION_COMMAND:New(coalition.side.BLUE, "Launch/Recover Aircraft (Turn CVN into the wind)",CVNMenu,CVN_start_recovery)
MENU_COALITION_COMMAND:New(coalition.side.BLUE, "Launch/Recover Aircraft (Turn LHA into the wind)",CVNMenu,LHA_start_recovery)
...
--- Patrol zone 1  
  if SetCVNActivePatrolZone == 1 and waypoint == 1 then
    MESSAGE:New(CVN_beacon_unit:GetName().. "proceeding to waypoint 2", msgTime):ToBlue()
    USERSOUND:New("ping.ogg"):ToCoalition(coalition.side.BLUE)
    BlueCVNGroup:GotoWaypoint(2)
  end
  
  if SetCVNActivePatrolZone == 1 and waypoint == 2 then
    MESSAGE:New(CVN_beacon_unit:GetName().. "proceeding to waypoint 3", msgTime):ToBlue()
    USERSOUND:New("ping.ogg"):ToCoalition(coalition.side.BLUE)    
    BlueCVNGroup:GotoWaypoint(1)
  end
