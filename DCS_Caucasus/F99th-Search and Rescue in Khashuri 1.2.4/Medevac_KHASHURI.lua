...
medevac.redmash = { "RedMASH #001", "RedMASH #002", "RedMASH #003", "RedMASH #004"} -- The unit that serves as MASH for the red side
...
medevac.sar_pilots = false -- Set to true to allow for Search & Rescue missions of downed pilots
...
medevac.maximumUnits = 3 -- If true weights is set to false, the heli can transport this many units
    ...
    local status, err = pcall(function(_event)
        ...
        elseif ((world.event.S_EVENT_EJECTION == _event.id and medevac.sar_pilots == true)) or (_event.id == 8) then
            ...
            elseif Object.hasAttribute(_unit, "Ground vehicles") then
				...
				-- Turned off spawning from vehicals that got blowed up.. for this mission - TracerFacer
                -- _spawnedGroup = medevac.spawnGroup(_unit, _isPilot)
...
function medevac.scheduledSARFlight(_args)
    ...
    local _status, _err = pcall(function(_args)
        ...
        if (_timeLeft < 1) then
			...
			local _redScore = trigger.misc.getUserFlag(800)
			_redScore = _redScore + 1
			trigger.action.setUserFlag(800, _redScore)
       ...
       if _dist < 200 and _heliUnit:inAir() == false then
            ...
            if medevac.clonenewgroups and _originalGroup ~= "" then
				...
				-- added by F99th-TracerFacer for scores VVVVVVVVVVVVVVVVVVVVVVVVVV
				--local _coa = Group.getCoalition(Group.getByName(_heliUnit))
				local _coa = _heliUnit:getCoalition()
				
				if _coa == 1 then --Russian
					local _redScore = trigger.misc.getUserFlag(800)
					_redScore = _redScore + 1
					trigger.action.setUserFlag(800, _redScore)
				...
				if _coa == 2 then --USA
					local _blueScore = trigger.misc.getUserFlag(801)
					_blueScore = _blueScore + 1
					trigger.action.setUserFlag(801, _blueScore)
				end
				scores()
            else
				...
				--added by S77Th-TracerFacer for scores VVVVVVVVVVVVVVVVVVVVVVVVVV
				--local _coa = Group.getCoalition(Group.getByName(_heliUnit))
				local _coa = _heliUnit:getCoalition()
			...
			--added by S77Th-TracerFacer for scores ^^^^^^^^^^^^^^^^^
            return
        end
...
env.info("Spawning random distressed units.")
function Spawn_distressed(_pos, _radius, _name, _infantry, _country)
	
	env.info("Spawning: " .. _name)
	env.info("For country: " .. _country)
	

	local _coord1, _coord2, _dist = coord.LOtoLL(_pos)
	local _randx = _pos.x + math.random(0 - _radius,_radius)
	local _randz = _pos.z + math.random(0 - _radius,_radius)
	local _tarpos = {x = _randx, z = _randz}
	local _idroot = math.random(1000, 10000)
	local _n = 1
	local _groupName = _name
	local _id = mist.getNextGroupId()	
	env.info("Next GroupID: " .. _id)
	local data = {
									   ["visible"] = false,
									   ["groupId"] = _id,
									   ["taskSelected"] = true,
	...
	coalition.addGroup(_country, Group.Category.GROUND, data) --Add the unit on the map
	medevac.injectWoundedGroup(_groupName,false)			  --Add the unit to the medevac list.

	--Set the crew hold fire ROE to HOLD
	if (medevac.crewholdfire) then 
		local _controller = Group.getByName(_groupName):getController() 
		_controller:setOption(0, 4) -- Weapon Hold
		_controller:setOption(9, 1) --radar off
	end
end					
...
function scores() 
	...
	local _message = ""
	local RedTickets = trigger.misc.getUserFlag(800)
	local BlueTickets = trigger.misc.getUserFlag(801)
	local MashUnits = trigger.misc.getUserFlag(802)

	_message = _message .. "==[ Rescue Operations Status ]==" .. "\n\n" 
	_message = _message .. "F99th Members Rescued: " .. BlueTickets .. "\n"
	_message = _message .. "F99th Members Lost   : " .. RedTickets .. "\n"
	_message = _message .. "Available Mash Units : " .. MashUnits .. "\n"

	trigger.action.outText(_message, 30)
end


-- This is the variable that holds the center of the dispersion of people to rescue
-- Example points:
_ZONE3 = {x=-00294401, z=00779440}
_ZONE2 = {x=-00291540, z=00774857}
_ZONE1 = {x=-00296969, z=00773314}


-- Type of unit that you are going to pick up
_UnitType = "Infantry AK"

-- Country of the unit (0 = Russia)
_CountryDistressed = 0 

--no red rescues for this mission. 
--Spawn_distressed(_ZONE1, _Dispersion, "F99th-Squid", _UnitType, _CountryDistressed)

_UnitType = "Soldier M4"
_CountryDistressed = 2

-- Size in meters of the area that the people are dispersed at
_Dispersion = 3000
Spawn_distressed(_ZONE1, _Dispersion, "F99th-Eightbit", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE1, _Dispersion, "F99th-JINX", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE1, _Dispersion, "ED's BAN List", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-TracerFacer", _UnitType, _CountryDistressed)
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Runny", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Pyromaniac", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Mataman", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE1, _Dispersion, "Dynamic's Whine", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE1, _Dispersion, "a box of ammo", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE1, _Dispersion, "Russ's lost MD80", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE1, _Dispersion, "F99th-Kaide", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE1, _Dispersion, "F99th-Ticman", _UnitType, _CountryDistressed) 

_Dispersion = 6000
Spawn_distressed(_ZONE2, _Dispersion, "F99th-Squid", _UnitType, _CountryDistressed)
Spawn_distressed(_ZONE3, _Dispersion, "Master Bates", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "Runny's Copy of Farm Simulator", _UnitType, _CountryDistressed)
Spawn_distressed(_ZONE3, _Dispersion, "ED's BAN List", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "Dynamic's Whine", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE2, _Dispersion, "F99th-Rounder", _UnitType, _CountryDistressed)
Spawn_distressed(_ZONE2, _Dispersion, "F99th-Molasses", _UnitType, _CountryDistressed)
Spawn_distressed(_ZONE2, _Dispersion, "F99th-Mohawk", _UnitType, _CountryDistressed)
Spawn_distressed(_ZONE2, _Dispersion, "Book: Tracer's Guide to Facing Tracer's", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE2, _Dispersion, "Shirt: Hillary for Prison", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE2, _Dispersion, "Mata's Taco Recipes", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE2, _Dispersion, "Cygon's Parrot", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE2, _Dispersion, "F99th-NastyNate", _UnitType, _CountryDistressed)
Spawn_distressed(_ZONE2, _Dispersion, "F99th-Kraut", _UnitType, _CountryDistressed)
Spawn_distressed(_ZONE2, _Dispersion, "F99th-Cook", _UnitType, _CountryDistressed)
Spawn_distressed(_ZONE2, _Dispersion, "F99th-Artic", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE2, _Dispersion, "F99th-Alphaskoom", _UnitType, _CountryDistressed) 

_Dispersion = 6000
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Cygon_Parrot", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Moose", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Toolbandit", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Tyler", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Frollo", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Russ", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Vampyre", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Hawss", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Korstat", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Narokuu", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "F99th-Cooper", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "Major Pain", _UnitType, _CountryDistressed)
Spawn_distressed(_ZONE3, _Dispersion, "Captain Obvious", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "Private Parts", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "Shadow's F99th Application", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "A MAGA Hat", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "Book: Russ's guide to bombing your own team", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "a box of ammo", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "Russ's lost MD80", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "my ability to fly this thing.", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "a steaming pile", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "Smegma", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "Eight Bits", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "V for Vampyre", _UnitType, _CountryDistressed) 
Spawn_distressed(_ZONE3, _Dispersion, "a Dirty Dingo", _UnitType, _CountryDistressed) 
